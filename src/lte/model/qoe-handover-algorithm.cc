...
#include "qoe-handover-algorithm.h"
#include <ns3/log.h>
#include <ns3/uinteger.h>
#include "ns3/core-module.h"
namespace ns3 {

NS_LOG_COMPONENT_DEFINE("QoeHandoverAlgorithm");

NS_OBJECT_ENSURE_REGISTERED(QoeHandoverAlgorithm);

///////////////////////////////////////////
// Handover Management SAP forwarder
///////////////////////////////////////////

QoeHandoverAlgorithm::QoeHandoverAlgorithm()
    : m_a2MeasId(0)
    , m_a4MeasId(0)
    ...
    , m_handoverManagementSapUser(0)
{
    ...
    m_handoverManagementSapProvider = new MemberLteHandoverManagementSapProvider<QoeHandoverAlgorithm>(this);
}
...
TypeId
QoeHandoverAlgorithm::GetTypeId()
{
    static TypeId tid = TypeId("ns3::QoeHandoverAlgorithm")
                            .SetParent<LteHandoverAlgorithm>()
                            .SetGroupName("Lte")
                            .AddConstructor<QoeHandoverAlgorithm>()
                            .AddAttribute("ServingCellThreshold",
                                ...
                                UintegerValue(30),
                                MakeUintegerAccessor(&QoeHandoverAlgorithm::m_servingCellThreshold),
                            ...
                            .AddAttribute("NeighbourCellOffset",
                                ...
                                UintegerValue(1),
                                MakeUintegerAccessor(&QoeHandoverAlgorithm::m_neighbourCellOffset),
                                MakeUintegerChecker<uint8_t>())
                            .AddAttribute("Threshold",
                                "lorem ipsum",
                                DoubleValue(0.2),
                                MakeDoubleAccessor(&QoeHandoverAlgorithm::m_threshold),
                            ...
                            .AddAttribute("TimeToTrigger",
                                ...
                                MakeTimeAccessor(&QoeHandoverAlgorithm::m_timeToTrigger),
                                MakeTimeChecker());
    return tid;
}

void QoeHandoverAlgorithm::SetLteHandoverManagementSapUser(LteHandoverManagementSapUser* s)
{
...
QoeHandoverAlgorithm::GetLteHandoverManagementSapProvider()
{
    NS_LOG_FUNCTION(this);
    return m_handoverManagementSapProvider;
}

void QoeHandoverAlgorithm::DoInitialize()
{
    NS_LOG_FUNCTION(this);
...
void QoeHandoverAlgorithm::DoDispose()
{
    NS_LOG_FUNCTION(this);
    delete m_handoverManagementSapProvider;
}

void QoeHandoverAlgorithm::DoReportUeMeas(uint16_t rnti,
...
void QoeHandoverAlgorithm::EvaluateHandover(uint16_t rnti,
    ...
    if (it1 == m_neighbourCellMeasures.end()) {
        ...
        for (int i = 0; i < n_c; ++i){
          if(cell[i][3] == servingCellId)
              NS_LOG_INFO("Célula " << cell[i][3] <<" -- Soma Qoe:" << soma[i] << " (serving)");
          else
              NS_LOG_INFO("Célula " << cell[i][3] <<" -- Soma Qoe:" << soma[i]);
...
bool QoeHandoverAlgorithm::IsValidNeighbour(uint16_t cellId)
{
...
void QoeHandoverAlgorithm::UpdateNeighbourMeasurements(uint16_t rnti,
    uint16_t cellId,
    uint8_t rsrq)
{
